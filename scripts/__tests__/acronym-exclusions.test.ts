...
describe("looksLikeAcronymName", () => {
  it.each(["MPC", "ADT", "AGL", "DAW", "AGLP", "AGSC", "VCA", "CMI", "EPS", "SEM"])(
    "flags true acronym '%s'",
    (name) => {
      expect(looksLikeAcronymName(name)).toBe(true);
    }
  );

  it.each(["REAPER", "TONIC", "VIRTUOSO", "EXOVERB", "ENRAGE", "AURORROR"])(
    "skips long stylized name '%s' (length cap)",
    (name) => {
      expect(looksLikeAcronymName(name)).toBe(false);
    }
  );

  it.each(["IRON", "LION", "MOOD", "BASE", "AMP", "ART", "MAP", "ONE", "TAILS", "ATONE"])(
    "skips short English word '%s' (exclusion list)",
    (name) => {
      expect(looksLikeAcronymName(name)).toBe(false);
    }
  );
  ...
  it.each(["Serum", "Pro-Q 3", "Massive X", "Ableton Live", "X"])(
    "skips ordinary product name '%s'",
    (name) => {
      expect(looksLikeAcronymName(name)).toBe(false);
    }
  );
